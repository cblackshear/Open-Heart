...
data A; set jhsv3.pfhb;run;
data A; set A;
if visit=9 then visit=3;
run;
      ...
      *create data set that has underlying risk score attributes;
	  data frs_chdtenyrriskOutliers;
		...
		*informat frs_chdtenyrrisk;
 		...
 		keep subjid sex age totchol hdl sbp diabetes eversmoker frs_chdtenyrrisk;
...
*Histograms for frs_cvdtenyrrisk;
	  ...
	  *create data set that has underlying risk score attributes;
	  data frs_cvdtenyrriskOutliers;
		set validation;
 		where frs_cvdtenyrrisk >= 0.63;
		keep subjid sex age totchol hdl sbp bpmeds diabetes eversmoker frs_cvdtenyrrisk;
		title1 'Participants with frs_cvdtenyrrisk score >= 0.63';
 ...
 *Histograms for frs_atpiii_tenyrrisk;
	  ...
	  *create data set that has underlying risk score attributes;
	  data frs_atpiii_tenyrriskOutliers;
 		...
 		keep subjid sex age totchol hdl sbp bpmeds eversmoker frs_atpiii_tenyrrisk;
 ...
 *Histograms for rrs_tenyrrisk;
	  ...
	  *create data set that has underlying risk score attributes;
	  data rrs_tenyrriskOutliers;
 		...
 		keep subjid sex age totchol hdl sbp diabetes eversmoker rrs_tenyrrisk;
 ...
 *Histograms for ascvd_tenyrrisk;
	  ...
	  *create data set that has underlying risk score attributes;
	  data ascvd_tenyrriskOutliers;
		set validation;
 		where ascvd_tenyrrisk >= 0.77;
 		keep subjid sex age totchol hdl sbp bpmeds diabetes eversmoker ascvd_tenyrrisk;
		title1 'Participants with ascvd_tenyrrisk score >= 0.77';
...
proc logistic data=validation;
	 class diabetes eversmoker sex;
	 model CHD(event='Yes')=age totchol hdl sbp diabetes eversmoker sex;
...
ods select none;
proc logistic data=validation;
	class eversmoker sex diabetes bpmeds;
	model CHD(event='Yes')=age totchol hdl sbp diabetes eversmoker sex bpmeds;
	score data=validation out=sco_validate;
run;
quit;
...
ods select none;
proc logistic data=validation;
   class eversmoker sex bpmeds;
   model CHD(event='Yes')=age totchol hdl sbp eversmoker sex bpmeds;
   score data=validation out=sco_validate;
run;
quit;
...
ods select none;
proc logistic data=validation ;
   class diabetes eversmoker sex parhistheartdis;
   model CHD(event='Yes')=age totchol hdl sbp diabetes eversmoker sex hscrp parhistheartdis HbA1c;
   score data=validation out=sco_validate;
run;
quit;
...
ods select none;
proc logistic data=validation;
	class bpmeds eversmoker sex diabetes;
	model CHD(event='Yes')=age totchol hdl sbp bpmeds eversmoker sex diabetes;
	score data=validation out=sco_validate;
run;
quit;
