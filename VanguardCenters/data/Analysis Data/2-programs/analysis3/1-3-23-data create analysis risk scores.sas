...
*Only include participants who have all covariates;
	...
	if missing(eversmoker) then delete;    *PROBLEM WITH 1-3-23 AND 1-2-23 IS SMOKER BECAUSE THAT VARIABLE IS NOT IN V2, CHANGE TO EVERSMOKER;
...
*For Males;
  ...
  *Framingham Point Scores by Smoking Status; 
  if sex="Male" & eversmoker=0 then smokescoremen=0;
  if sex="Male" & eversmoker=1 then smokescoremen=2;
...
*For Females;
  ...
  *Framingham Point Scores by Smoking Status; 
  if sex="Female" & eversmoker=0 then smokescorefe=0;
  if sex="Female" & eversmoker=1 then smokescorefe=2;
...
*Only include participants who have all covariates;
	...
	if missing(sbp) then delete;
	if missing(diabetes) then delete;
	if missing(eversmoker) then delete;
	if missing(sex) then delete;
...
*For Males;
  ...
  *Framingham Point Scores by Smoking Status;
  if sex="Male" & eversmoker=0 then smokescoremen=0;
  if sex="Male" & eversmoker=1 then smokescoremen=4;
...
*For Females;
  ...
  *Framingham Point Scores by Smoking Status; 
  if sex="Female" & eversmoker=0 then smokescorefe=0;
  if sex="Female" & eversmoker=1 then smokescorefe=3;
...
*For Males;
  ...
  *Creating smoke score;
  if eversmoker=0 and agecategory=1 then smokescore=1;
  if eversmoker=0 and agecategory=2 then smokescore=2;
  if eversmoker=0 and agecategory=3 then smokescore=3;
  if eversmoker=0 and agecategory=4 then smokescore=4;
  if eversmoker=0 and agecategory=5 then smokescore=5;

  if eversmoker=1 and agecategory=1 then smokescore=6;
  if eversmoker=1 and agecategory=2 then smokescore=7;
  if eversmoker=1 and agecategory=3 then smokescore=8;
  if eversmoker=1 and agecategory=4 then smokescore=9;
  if eversmoker=1 and agecategory=5 then smokescore=10;
...
/*data E; set jhsv1.pfha; run;*/
data F; set jhsv3.pfhb; run;


*pfhb data is for visit 3, so change visit "9" to visit "3";
data F; set F;
if visit=9 then visit=3;run;


*Sort data sets before merging;
proc sort data=D; by subjid; run;
/*proc sort data=E; by subjid visit; run;*/
proc sort data=F; by subjid visit; run;

*Merge the three data sets; 
data include4;
  merge D (in=in1) F;
  by subjid;
  if in1=1;    *Only keep participants in visit3;
run;
...
*Male Covariates;
  ...
  *conditional smoking term;
  if eversmoker=1 then smoketermen = 0.405;
  if eversmoker=0 then smoketermen = 0;
...
*Female Covariates;
  ...
  *Interaction terms;
  *conditional smoking term;
  if eversmoker=1 then smoketermwmn = 0.818;
  if eversmoker=0 then smoketermwmn = 0;
